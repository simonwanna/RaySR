...
def generate_global_building_mask(
        scene: "Scene",
        height_map: np.ndarray,
        global_grid_info: dict,
        config: TransmitterConfig
    ) -> tuple[np.ndarray, np.ndarray]:
    """Generate global building mask and ground height map"""
    # Generate ground height map

    grid_info = {key: val for key, val in global_grid_info.items() if key in ["xmin", "xmax", "ymin", "ymax", "nx", "ny"]}

    ground_height_map = height_map_ray_casting(scene, grid_info, direction=(0.0, 0.0, 1.0))
...
def generate_global_grid_info(scene: "Scene", config: TransmitterConfig) -> dict:
    ...
    height_map = height_map_ray_casting(scene, global_grid_info, direction=(0.0, 0.0, -1.0))
    building_mask, ground_height_map = generate_global_building_mask(scene, height_map, global_grid_info, config)
    
    global_grid_info["global_height_map"] = height_map
    global_grid_info["building_mask"] = building_mask
    global_grid_info["ground_height_map"] = ground_height_map
...
def generate_grid_positions(
    ...
    global_building_mask = global_grid_info["building_mask"]
    global_ground_height_map = global_grid_info["ground_height_map"]
    ...
    grid_building_mask = global_building_mask[
        row_bottom_index:row_top_index,
        col_left_index:col_right_index
    ]

    grid_ground_height_map = global_ground_height_map[
        row_bottom_index:row_top_index,
        col_left_index:col_right_index
    ]
    ...
    grid_info = {
        ...
        "grid_building_mask": grid_building_mask,
        "grid_ground_height_map": grid_ground_height_map,
    }
...
def generate_dataset(
    ...
    for i in iterator:
        ...
        sample = generate_sample(scene, i + 1, config, tx_grid_info, global_grid_info)
